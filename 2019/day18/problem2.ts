...
function availableKeys(
  grid: string[][],
  queue: GridState[],
  seen = new Set<string>(queue.map(state => posToStr(state.pos))),
  keys: DestKeys = {}
) {
...
function getValidNextPaths(nextPaths: DestKeys, currentKeys: string[]) {
  return Object.keys(nextPaths).filter(newKey => {
    ...
    return nextPaths[newKey].reqKeys.every(reqKey => {
      return currentKeys.includes(reqKey)
    })
  })
}
...
function solveGrid(grid: string[][]) {
  ...
  const pos = Array(count).fill(0).map((_, i) => {
    return positions.getVal(i.toString())!
  })
  return dfsPaths(
    { keys: [], steps: 0, pos },
    paths,
