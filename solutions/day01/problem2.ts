...
// The easy answer to the second question is search and replace
// Replace all text values with numbers, then just run it through the same thing

function replaceWithNums(str: string): string {
  // Theoretically, we only need to replace the first and last number in the string
  // And we only have to do that if there isn't a real digit first
  //
  // Alternatively, can we detect collisions?
  const nums = {
    'one': '1',
    'two': '2',
    'three': '3',
  ...
  for (const num in nums) {
    while (result.includes(num)) {
      // str.indexOf(num)
      // console.log(str, 'INCLUDES', num)
      result = result.replace(num, nums[num])
      // console.log(result)
    }
  }
  return result;
...
let total = 0;
// for (const str of exampleCase) {
for (const str of testData) {
// for (const str of exampleCase2) {
  const nums = {
    'one': '1',
    'two': '2',
    'three': '3',
  ...
  // Get first and last digit of each string, put those two numbers next to each other, and add them to the total
  // console.log(str)
  // const newStr = replaceWithNums(str)
  // console.log(newStr)
  const chars = str
  // const chars = newStr.split('')
  let firstDigit, lastDigit;
  for (let i = 0; i < chars.length; i++) {
    // console.log('checking', chars[i])
    for (const key in nums) {
      if (chars[i] === key[0]) {
        // CHECK IF THE REST OF THE STRING MATCHES
        // console.log('slice from ', i, ' to ', key.length)
        // console.log(chars.slice(i, key.length))
    ...
    if (chars[i] >= '0' && chars[i] <= '9') {
      // console.log('FIRST CHAR', chars[i]);
      firstDigit = chars[i]
      break;
    }
  }
  for (let i = 1; i <= chars.length; i++) {
    // REWRITE THIS SO IT WORKS BACKWARDS
    const realIndex = chars.length - i;
    //console.log('CHECKING', chars[realIndex])
    for (const key in nums) {
      if (chars[realIndex] === key[key.length - 1]) {
        // const stringContent = chars.slice(realIndex - key.length, realIndex);
        const stringContent = chars.slice(realIndex - key.length + 1, realIndex + 1);
        // console.log('key', key, 'content', stringContent)
    ...
    if (lastDigit) break;
    // console.log('searching for last char', chars[chars.length - i])
    if (chars[chars.length - i] >= '0' && chars[chars.length - i] <= '9') {
      // console.log('LAST CHAR', chars[chars.length - i]);
  ...
  if (isNaN(total)) {
    console.log(str)
    console.log('YOURE FUCKED', firstDigit, lastDigit)
    break;
  }
  // console.log(total)
}
